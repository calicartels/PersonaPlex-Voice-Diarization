...
# PyTorch requires nn.Module for trainable models. Only class in the project.
class MimiSpeaker(nn.Module):
    def __init__(self, input_dim=512):
        super().__init__()
        self.proj = nn.Linear(input_dim, D_MODEL)
        # Choice: PyTorch TransformerEncoder (built-in, well-tested).
        # Alternative: Fast-Conformer (Sortformer uses it), adds complexity
        # with no benefit at 4-layer scale.
        layer = nn.TransformerEncoderLayer(
            d_model=D_MODEL, nhead=N_HEADS, dim_feedforward=FF_DIM,
            dropout=DROPOUT, batch_first=True,
            # Choice: GELU (modern default). Alternative: ReLU, marginal difference.
            activation="gelu",
        )
