...
def upload_to_hf(repo_id, checkpoint_path, token=None):
    ...
    api.upload_file(
        ...
        path_in_repo=f"encoder_fusion/checkpoints/{checkpoint_path.name}",
        repo_id=repo_id,
        repo_type="model",
    )
...
if not repo_id:
    repo_id = "TMVishnu/personaplex-voice-diarization"
token = os.getenv("HF_TOKEN")
if upload_best(repo_id, token):
    print(f"Uploaded best.pt to {repo_id}")
else:
    print("Upload skipped: best.pt not found or HF_REPO not set")
